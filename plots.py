...
for r in grid:
    ...
    w_var = None
    try:
        ef_mv = EfficientFrontier(mu, S) # builds an optimizer with mean-variance objectives
        ef_mv.efficient_return(r) # solve subject to return = r and sum of weights = 1
        w_mv = as_series(ef_mv.clean_weights(), mu.index) # cleans, sorts and organizes weights
        ret_mv, vol_mv, _ = ef_mv.portfolio_performance() # the function returns expected return, volatility, and sharpe
    except Exception:
        continue

    # CVaR
    try:
        ef_c = EfficientCVaR(mu, rets) # builds an optimizer with CVaR objectives
    ...
    try:
        result = minimize(
            ...
            initial_guess,
            args=(rets,),
            method='SLSQP', # can handle constraints like sum of weights = 1
            bounds=bounds,
            constraints=[ # scipy opt requires constraints to be defined as lists
                {'type': 'eq', 'fun': lambda weights: np.sum(weights) - 1}, # sum of weights = 1
                {'type': 'eq', 'fun': lambda weights: portfolio_return(weights, mu) - r} # target return = r
...
if common_pts:  # check list is not empty
    # Sort by expected return
    sorted_portfolios = sorted(common_pts, key=lambda p: p["ret"])
    ...
    fig.suptitle("Portfolio Value Evolution Over Time (Common Points)", fontsize=16)
    axes = axes.flatten()

    for i, (title, portfolio_data) in enumerate(portfolios_to_plot):
        ax = axes[i]
        # Unpack dict
        vol = portfolio_data["mv_risk"]
        ret = portfolio_data["ret"]
        w = portfolio_data["w"]  # pandas Series
